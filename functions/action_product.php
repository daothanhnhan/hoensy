...
public function getProductList_byMultiLevel_orderProductId_sp_hot($idProductCat,$order_productId,$trang, $limit, $page){
...
public function getProductList_byMultiLevel_orderProductId_dv_hot($idProductCat,$order_productId,$trang, $limit, $page){
...
/*------ lấy thông tin các sản phẩm liên quan -----*/
public function getListProduct_byIdCat($idProductCat){
    global $conn_vn;
    $table = $this->nameTable_product;
    // $where = "where $this->nameColId_productCat = '".$idProductCat."'";
    $where = "where productcat_ar LIKE '%".$idProductCat."%'";
...
public function getMinPrice(){
    global $conn_vn;
    $sql = "SELECT MIN(product_price) FROM product";
    $result = mysqli_query($conn_vn,$sql);
    if (mysqli_num_rows($result) > 0){
        $row = mysqli_fetch_assoc($result);
        return $row;
