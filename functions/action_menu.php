...
class action_menu extends library{
    ...
    /*-----------------------------*/
    public function showMenu_byMultiLevel_mainMenuTranhTrangTri($list_menu,$valParent_menu = 0,$lang, $level = 0){
        global $rowConfig;
        $cate_child = array();
        foreach ($list_menu as $key => $item)
        {
        ...
        if ($cate_child) {    
            echo '<ul>';            
            // echo '<li><a href="/" class="head-menu-icon"><img src="/images/'.$rowConfig['web_logo'].'" class="logo-menu"></a></li>';
            foreach ($cate_child as $key => $item) {
                $row = $this->getMenuLanguageDetail_byId($item[$this->nameColID_menu],$lang);
                $name = $row[$this->nameColTitle_menuLanguage];
                $url = $this->setUrlFriendly_byType($row[$this->nameColIdMenu_menuLanguage],$lang);                
            ...
            echo '<li><a id="search"><i class="fa fa-search" aria-hidden="true"></i></a><div class="gb-box-search">
                    <div class="gb-input-group">
                        <form action="/search-product/0" method="post">
                            <input type="text" name="q" placeholder="Tìm kiếm khóa học..." class="form-control">
            ...
            echo '<li><a href="https://www.facebook.com/hoensy" target="_blank" class="head-menu-icon"><i class="fa fa-facebook-official" style="color:#3C5A99;"></i></a></li>';
            echo '<li><a href="https://www.youtube.com/channel/UCReTNSstPQfHHFB2B45iNxg" target="_blank" class="head-menu-icon"><i class="fa fa-youtube-play" style="color:#F54236;"></i></a></li>';
    ...
    public function showMenu_byMultiLevel_mainMenuTranhTrangTri_m($list_menu,$valParent_menu = 0,$lang, $level = 0){
        $cate_child = array();
        foreach ($list_menu as $key => $item)
        {
            // Nếu là chuyên mục con thì hiển thị
        ...
        if ($cate_child) {    
            ...
            if ($level == 0) {
                if (empty($_SESSION['user_id_gbvn'])) {
                    echo '<li><a href="/login" title="">Đăng nhập</a></li>';
                } else {
                    echo '<li><a href="/thong-tin-ca-nhan" title="">Khóa học đang sở hữu</a></li>';
                }
